/**
 * TitleScene responsive layout testing
 * Tests main menu display and background resize across multiple viewports
 */
...
async function testTitleScene() {
    ...
    console.log('Starting TitleScene layout tests...\n');
    ...
    for (const viewport of viewports) {
        ...
        try {
            // Navigate to app (TitleScene loads by default)
            await page.goto('http://127.0.0.1:5500/index.html', { waitUntil: 'load', timeout: 10000 });

            // Wait for Phaser to initialize and TitleScene to load
            await page.waitForTimeout(2500); // Extra time for animations

            // Take screenshot
            const filename = `${viewport.name}_title.png`;
